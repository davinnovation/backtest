...
def app():
    ...
    st.subheader(
        f"taxed buy and hold {result[-1].all_sell_profit_percent(sell_years, tax_free=2500*len(years), tax_ratio=0.154)}")

    st.subheader(
        f"taxed monthly buy {result2[-1].all_sell_profit_percent(sell_years, tax_free=2500*len(years), tax_ratio=0.154)}")

    if isa_mode:
        st.subheader(
            f"isa buy and hold {result[-1].all_sell_profit_percent(sell_years, tax_free=2000, tax_ratio=0.09)}")

        st.subheader(
            f"isa monthly buy {result2[-1].all_sell_profit_percent(sell_years, tax_free=2000, tax_ratio=0.09)}")
